...
describe("Admin Controllers", () => {
    ...
    it("should load /admin/settings/homepage", done => {
        request(
            `${nconf.get("url")}/api/admin/settings/homepage`,
            { jar: jar, json: true },
            (err, res, body) => {
                assert.ifError(err);
                assert.equal(res.statusCode, 200);
    ...
    it("should load /admin/settings/languages", done => {
        request(
            `${nconf.get("url")}/api/admin/settings/languages`,
            { jar: jar, json: true },
            (err, res, body) => {
                assert.ifError(err);
                assert.equal(res.statusCode, 200);
    ...
    it("should load /admin/settings/social", done => {
        const socketAdmin = require("../src/socket.io/admin");
        socketAdmin.social.savePostSharingNetworks(
            { uid: adminUid },
            ["facebook", "twitter"],
            err => {
                assert.ifError(err);
                request(
                    `${nconf.get("url")}/api/admin/settings/social`,
                    { jar: jar, json: true },
                    (err, res, body) => {
                        assert.ifError(err);
                        assert(body);
